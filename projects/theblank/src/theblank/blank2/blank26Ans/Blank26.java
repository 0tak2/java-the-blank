...
class TestStup2 {
	public ArrayList<Menu> getMenuList() {
		...
		list.add(new Menu("BigMac", "Big Big One", 5, null,
				new Coupon25()));
		...
		while(!(query = sc.next()).equals("exit")) {
			for (Menu menu : list) {
				if (query.equals(menu.getMenuName())) {
					...
					Coupon coupon = menu.getCoupon();
					if (coupon != null) {
						System.out.println("\nA Coupon available.");
						System.out.println("Price after applying the coupon: " + coupon.calcCouponPrice(menu));
					}
					break;
				}
			}
